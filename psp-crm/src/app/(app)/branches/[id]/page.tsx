...
import { fmtCurrency, fmtCurrencyShort, fmtDeltaPct, fmtPct, fmtDate, whiteSpaceLabel } from '@/lib/format';
...
export default async function BranchDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Link href={`/accounts/${branch.account_id}`} className="text-sm text-brand-700 hover:underline">
        ← Account
      </Link>
      <div className="mt-2 flex flex-wrap items-center gap-3">
        <h1 className="text-xl font-bold tracking-tight text-charcoal">{branch.branch_name}</h1>
        <StatusBadge status={branch.status} />
        <RagBadge rag={branch.coverage_rag} />
        <span className="text-sm text-muted">{[branch.city, branch.state].filter(Boolean).join(', ')}</span>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <KpiTile label="White-space" value={whiteSpaceLabel(branch.white_space)} tone={branch.white_space === '—' ? 'good' : 'warn'} />
      </div>
            ...
            <tbody>
              {txns.map((t) => (
                <tr key={t.id} className="border-b border-line/60 last:border-0">
                  ...
                  <td className="px-4 py-2 text-muted">{t.invoice_nbr ?? t.so_nbr ?? '—'}</td>
                  <td className="px-4 py-2">{t.inventory_description ?? t.inventory_id ?? '—'}</td>
                  <td className="px-4 py-2">
                    <span className="rounded bg-canvas px-1.5 py-0.5 text-xs">{t.product_line}</span>
                  </td>
                  <td className="px-4 py-2 text-right tabular-nums">{fmtCurrency(t.net_sale)}</td>
                  <td className="px-4 py-2 text-right tabular-nums text-muted">{fmtCurrency(t.margin)}</td>
                  <td className="px-4 py-2 text-muted">{t.status}</td>
                </tr>
              ))}
              {txns.length === 0 && (
                <tr>
